...
InitialCell = Constant(-0.5758) #conditional(And(31 <= x, x < 39)), Constant(2.0), Constant(-0.5758)
...
if RK == True:
    ...
    butcher_tableau = RadauIIA(1)  #LobattoIIIC(2)
...
#inner((chi/eps) * (uCurr - (uCurr**3 / 3) - cCurr), vu)*dx
    Fu = inner(chi * capacitance * Dt(uCurr), vu)*dx + inner(grad(uCurr), sigma * grad(vu))*dx + inner((chi/eps) * (-uCurr + (uCurr**3 / 3) + cCurr), vu)*dx
    Fc = inner(Dt(cCurr), vc)*dx - inner(eps * uCurr, vc)*dx - inner(beta * eps, vc)*dx + inner(gamma * eps * cCurr, vc)*dx
    F = Fu + Fc
    ...
    outfile1 = File("FHN_results/FHN_1d_u.pvd")
    outfile2 = File("FHN_results/FHN_1d_c.pvd")
...
d = 1.0
    ...
    InitialPotential = conditional(And(And(And(70 <= x, x < 80), And(0 <= y, y < 35)), t < 0.0002), Constant(1.0), Constant(0))
