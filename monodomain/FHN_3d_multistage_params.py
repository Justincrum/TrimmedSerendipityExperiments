...
msh = RectangleMesh(100, 100, 70, 70, quadrilateral=True)
mesh = ExtrudedMesh(msh, layers=100, layer_height=0.7)
polyOrder = 1
...
# InitialCell = conditional(And( And(31 <= x, x < 39), And(And(0 <= y, y < 35), And(0 <= z, z < 35))),
#                           Constant(2.0), Constant(-0.5758))
InitialCell = conditional(And(And(31 <= x, x < 39), And(0 <= z, z < 35)),
                          Constant(2.0), Constant(-0.5758))
...
sigma = as_matrix([[sigma1, 0.0, 0.0], [0.0, sigma2, 0.0], [0.0, 0.0, sigma3]])
...
outfile1 = File("FHN_results/FHN_3d_u.pvd")
outfile2 = File("FHN_results/FHN_3d_c.pvd")
